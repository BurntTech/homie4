...
class Device_Base(object):
    ...
    def publish_homeassistant(self):
        ...
        HASS_PAYLOADS = {
            ...
            #'dimmer': f'{{"name": "{self.name}","schema": "template","command_topic": "homie/{self.device_id}/dimmer/dimmer/set","state_topic": "homie/{self.device_id}/dimmer/dimmer","state_value_template": "{{{{ \'off\' if value == \'0\'  else \'on\' }}}}","command_on_template": "{{{{ ((value | int / 255 * 100) | round(0)) if value is defined else 255 }}}}","command_off_template": "0","brightness_template": "{{{{ ((value | int) / 100 * 255) | round(0) }}}}"}}',
            'dimmer': f'{{"name": "{self.name}","command_topic": "homie/{self.device_id}/dimmer/dimmer/set","brightness_command_topic": "homie/{self.device_id}/dimmer/dimmer/set","brightness_state_topic": "homie/{self.device_id}/dimmer/dimmer","state_topic": "homie/{self.device_id}/dimmer/dimmer","state_value_template": "{{{{ 0 if (value | int) == 0 else 100 }}}}","brightness_scale": 100,"on_command_type": "brightness","brightness_value_template": "{{{{ value | int }}}}","payload_on" : 100,"payload_off" : 0}}',
